...
HOME_DEG = np.array([90.0, -90.0, 90.0, -90.0, -90.0, 0.0])

# Very small motion: only rotate wrist_3 by 5 degrees.
# This should mostly rotate the tool flange instead of sweeping the whole arm.
ALT_DEG = np.array([105.0, -70.0, 90.0, -110.0, -90.0, 15.0])
...
def main():
    ...
    print(f"Alt  deg: {ALT_DEG.tolist()}")
    ...
    try:
        with mujoco.viewer.launch_passive(
            ...
            while viewer.is_running():
                ...
                use_alt = int(elapsed // SWITCH_PERIOD_S) % 2 == 1
                target_q = alt_q if use_alt else home_q

                if USE_REAL_ROBOT:
                    command_socket.send_json({"target_q": target_q.tolist()})
                ...
                if robot_q is not None and robot_q.shape[0] >= 6:
                    ...
                    display_q = 0.92 * display_q + 0.08 * target_q
